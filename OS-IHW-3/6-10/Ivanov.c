...
void sendHandleRequest(int client_socket, struct Task task) {
    ...
    if (task.status == 0) {
        printf("Ivanov took item #%d from the storage\n", task.index + 1);
    }
    if (task.status == 1) {
        printf("Ivanov passed item #%d over to Petrov\n", task.index + 1);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    int number;
    if ((bytes_received = recv(client_socket, &number, sizeof(int), 0)) != sizeof(int)) {
        perror("recv() bad");
        exit(-1);
    }

    work(client_socket, number);
    printf("Ivanov's work is completed\n");
