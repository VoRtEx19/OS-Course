...
void sendHandleRequest(int client_socket, struct Task task) {
    ...
    if (task.status == 0) {
        printf("Petrov received item #%d that is worth %d from Ivanov\n", task.index + 1, status);
    }
    if (task.status == 1) {
        printf("Petrov loaded item #%d in the truck\n", task.index + 1);
    }
}
...
void work(int client_socket, int number) {
	...
	for (int i = 0; i < number; ++i) {
		sleep(rand() % 3 + 1);
		task.status = 0; // indicates taking item from Ivanov
		task.index = i;
		sendHandleRequest(client_socket, task);
		sleep(rand() % 3 + 1);
		task.status = 1; // indicates putting item in the truck
		sendHandleRequest(client_socket, task);
	}
	task.index = -1;
...
int main(int argc, char *argv[]) {
    ...
    printf("Petrov's work is completed\n");
